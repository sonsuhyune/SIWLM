...
def compute_prf(predicts, labels):
    (p, r, f, s) =  precision_recall_fscore_support(labels, predicts, average='macro')
...
def calc_metrics(metric_meta, golds, predictions, scores, label_mapper=None):
    ...
    for mm in metric_meta:
        ...
        if mm in (Metric.ACC, Metric.F1, Metric.MCC, Metric.F1MAC, Metric.F1MIC, Metric.PRF):
